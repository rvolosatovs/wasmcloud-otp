      ...
      mkFlake {
        ...
        withDevShells = {
          ...
          build-mac-erts = pkgs.writeShellScriptBin "build-mac-erts" ''
            ...
            rm -f $out/erts-aarch64-apple-darwin-mac.tar.gz
            rm -f $out/erts-x86_64-apple-darwin-mac.tar.gz

            mv $out/result $out/erts-aarch64-apple-darwin-mac.tar.gz
            mv $out/result-1 $out/erts-x86_64-apple-darwin-mac.tar.gz
          '';
          ...
          release = pkgs.writeShellScriptBin "release" ''
            ...
            printf "''${SECRET_KEY_BASE}" | nix build -L \
            ...
            rm -f $out/host_core-*
            rm -f $out/wasmcloud_host-*

            mv $out/result    $out/host_core-burrito-aarch64-darwin
            mv $out/result-1  $out/host_core-burrito-aarch64-linux-gnu
            mv $out/result-2  $out/host_core-burrito-aarch64-linux-musl
            mv $out/result-3  $out/host_core-burrito-x86_64-darwin
            mv $out/result-4  $out/host_core-burrito-x86_64-linux-gnu
            mv $out/result-5  $out/host_core-burrito-x86_64-linux-musl
            mv $out/result-6  $out/host_core-burrito-x86_64-windows

            mv $out/result-7  $out/wasmcloud_host-burrito-aarch64-darwin
            mv $out/result-8  $out/wasmcloud_host-burrito-aarch64-linux-gnu
            mv $out/result-9  $out/wasmcloud_host-burrito-aarch64-linux-musl
            mv $out/result-10 $out/wasmcloud_host-burrito-x86_64-darwin
            mv $out/result-11 $out/wasmcloud_host-burrito-x86_64-linux-gnu
            mv $out/result-12 $out/wasmcloud_host-burrito-x86_64-linux-musl
            mv $out/result-13 $out/wasmcloud_host-burrito-x86_64-windows
          '';
        in
          extendDerivations {
